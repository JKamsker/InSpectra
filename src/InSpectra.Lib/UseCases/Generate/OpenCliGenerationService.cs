...
internal sealed class OpenCliGenerationService(
    ...
    : IOpenCliGenerationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            allWarnings.Add("Crawl artifact output was requested, but the selected acquisition mode did not produce crawl data.");
        }
    ...
    private static void EnsureDistinctArtifactPaths(string? outputFile, string? crawlOutputPath)
    {
        ...
        throw new CliUsageException("OpenCLI output and crawl output must point to different files.");
    }
