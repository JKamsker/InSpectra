...
using InSpectra.Gen.Engine.Tests.StaticAnalysis;
...
public sealed class OpenCliMetadataContractTests
{
    ...
    {
        var builder = new InSpectra.Gen.Engine.Modes.Static.Projection.StaticAnalysisOpenCliBuilder();
        var document = builder.Build(
            "demo",
            "1.0.0",
            "System.CommandLine",
            new Dictionary<string, InSpectra.Gen.Engine.Modes.Static.Metadata.StaticCommandDefinition>(StringComparer.OrdinalIgnoreCase),
            new Dictionary<string, Document>(StringComparer.OrdinalIgnoreCase));
